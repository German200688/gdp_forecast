...

{
	...
	for (int32_t i1 = 0; i1 < t2; i1++)
	{
		...
		delta[i1] = obj2.deltaMiddle(t1, Weights3[i1], delta3, alpha, Outputs[i1]);
...
void layer2::teachdel2ma(vector<signed char >& delta3, int32_t& Quoter, vector<vector<signed char>>& Weights3, vector<vector<signed char>>& Weights2, vector<vector<signed char>>& Outputs2, vector<signed char >& delta2, signed char*& vec_a, signed char*& vec_b, signed char*& vec_c, signed char*& vec_d)

{
	...
	//int32_t t0 = Weights3.size();
	int32_t t011 = Outputs2[Quoter].size();
	vector<signed char > delta(t011);
	signed char* deltama = delta.data();

	signed char* Outputs = Outputs2[Quoter].data();
	signed char* Weightsl = Weights3[0].data();
...
void layer2::teach2ma(int32_t& Quoter, vector<vector<signed char>>& Outputs2, signed char alpha, vector<vector<signed char>>& Weights2, vector<signed char >& delta2, signed char*& vec_d, signed char*& vec_a)
